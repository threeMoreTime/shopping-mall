  ...
  <view class="bg" :class="[tabIndex === 2 ? 'bg2' : '']">
    <view class="box" :class="[tabIndex === 2 ? 'box2' : '']">
      ...
      <view class="tabs">
        <view
          class="tabItem"
          :class="[tabIndex === index ? 'tabItemActivate' : '']"
          v-for="(item, index) in tabs"
          :key="item.id"
          @click="changleStyle(index)"
        >
          <text>{{ item.name }}</text>
        </view>
      </view>
      ...
      <view class="buyList" v-show="tabIndex === 1">
        <view class="buyListItem">
          <view class="buyListItemTitle">
            <view class="ItemTitle">数量23.00</view>
            <view class="ItemPrice">
              <text style="margin-right: 22rpx; text-decoration: line-through"
                >单价￥6.42</text
              >
              <text>￥147.88</text>
            </view>
      ...
      <u-popup v-model="isPopupShow" mode="bottom" border-radius="16">
        <view class="popClass">
          <view class="inpItem" v-show="showIndex === 0">
            <u-input
              v-model="dataForm.numberTransaction"
              type="number"
              height="88"
              placeholder="请输入交易数量"
            />
